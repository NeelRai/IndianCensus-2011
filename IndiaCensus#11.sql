...
-- if table with same name exits drop it
drop table if exists #topstates;
-- temporary table with 2 columns
...
--union of topstates and bottomstates table
...
select distinct state from project1..Data1 where lower(state) like 'a%' and lower(state) like '%m'

-- joining both table

--total males and females in the states
...
-- total no. of literate people and illiterate people
...
inner join project1..Data2 b on a.district=b.district) d) c
group by c.state

-- population in previous census vs current census camparison
